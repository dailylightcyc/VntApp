...
impl SyncDevice {
    ...
    /// Android's VpnService supplies an fd that VNT handles as a fallible
    /// conversion. Keep the 2.0.3 Linux behavior while exposing that API only
    /// on Android.
    #[cfg(target_os = "android")]
    pub unsafe fn from_fd(fd: RawFd) -> std::io::Result<Self> {
        let device = SyncDevice(DeviceImpl::from_fd(fd));
        // Android's VpnService can supply either a blocking or non-blocking fd.
        // VNT uses SyncDevice and treats WouldBlock as a fatal TUN read error, so
        // normalize the descriptor at this ownership boundary. This remains
        // safely interruptible when the `experimental` shutdown feature is on.

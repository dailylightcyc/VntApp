...
bool Tray::init_indicator_api() {
  ...
  do {
    ...
    // CMake prefers Ayatana when it is installed. Loading libappindicator here
    // after linking libayatana-appindicator mixes two GObject implementations
    // in one process and can crash in app_indicator_set_status(). Always load
    // the same ABI that this plugin was compiled and linked against.
#ifdef HAVE_AYATANA
    constexpr char kAppIndicatorLibrary[] =
        "libayatana-appindicator3.so.1";
#else
    constexpr char kAppIndicatorLibrary[] = "libappindicator3.so.1";
#endif
    void* handle = dlopen(kAppIndicatorLibrary, RTLD_NOW | RTLD_LOCAL);
    if (!handle) {
      break;
    }

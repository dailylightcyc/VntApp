...
import top.daylight.vnt.www.vpn.IpUtils;
import top.daylight.vnt.www.vpn.MyVpnService;

public class FlutterMethodChannel {
    private static final String CHANNEL = "top.daylight.vnt.www/vpn";
                    ...
                    switch (call.method) {
                        case "startVpn":
                            ...
                            try {
                                ...
                                NativeLogger.error("FlutterChannel", "invalid VPN configuration", e);
                            ...
                            try {
                                ...
                                NativeLogger.error("FlutterChannel", "startVpn callback failed", e);
    ...
    public static void callSuccess(int fd) {
        NativeLogger.info("FlutterChannel", "returning VPN fd=" + fd + " to Rust core");
    ...
    public static void callError(String msg, Exception e) {
        NativeLogger.error("FlutterChannel", msg, e);
        if (pendingResult != null) {
    ...
    public static void startVnt(String configKey, Function<Boolean, Void> function) {
        ...
        channel.invokeMethod("startVnt", configKey, new MethodChannel.Result() {
            ...
            public void error(@NonNull String errorCode, @Nullable String errorMessage, @Nullable Object errorDetails) {
                NativeLogger.error("FlutterChannel", "startVnt failed: " + errorMessage, null);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
    ...
    public static void isRunning(Function<Boolean, Void> function) {
        ...
        channel.invokeMethod("isRunning", null, new MethodChannel.Result() {
            ...
            @Override
            public void error(@NonNull String errorCode, @Nullable String errorMessage, @Nullable Object errorDetails) {
                NativeLogger.error("FlutterChannel", "isRunning failed: " + errorMessage, null);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    function.apply(false);
                }
            }
    ...
    public static void getDeviceInfo(Function<Map<String, Object>, Void> function) {
        ...
        channel.invokeMethod("getDeviceInfo", null, new MethodChannel.Result() {
            ...
            @Override
            public void error(@NonNull String errorCode, @Nullable String errorMessage, @Nullable Object errorDetails) {
                NativeLogger.error("FlutterChannel", "getDeviceInfo failed: " + errorMessage, null);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    Map<String, Object> emptyInfo = new HashMap<>();
                    emptyInfo.put("isConnected", false);
                    emptyInfo.put("configName", "");
    ...
    public static void updateWidgetAndTileState(Context context, boolean isConnected) {
        NativeLogger.debug("FlutterChannel", "更新磁贴和小组件状态: isConnected=" + isConnected);

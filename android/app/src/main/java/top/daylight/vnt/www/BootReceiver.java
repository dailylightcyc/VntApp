...
public class BootReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (Intent.ACTION_BOOT_COMPLETED.equals(intent.getAction())) {
            ...
            if (autoStart) {
                NativeLogger.info(TAG, "开机自启已启用，准备启动应用");
                ...
                try {
                    ...
                    NativeLogger.info(TAG, "应用启动成功");
                } catch (Exception e) {
                    NativeLogger.error(TAG, "启动应用失败", e);
                }
            } else {
                NativeLogger.debug(TAG, "开机自启未启用");
            }
        }
    }
}

...
public abstract class VntWidget extends AppWidgetProvider {
    ...
    protected static final String ACTION_TOGGLE = "top.daylight.vnt.www.WIDGET_TOGGLE";
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        NativeLogger.debug(TAG, "onUpdate: 更新小组件");
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION_TOGGLE.equals(intent.getAction())) {
            NativeLogger.info(TAG, "收到切换连接状态的请求");
    ...
    private void handleToggle(Context context) {
        ...
        if (!FlutterMethodChannel.initialized()) {
            NativeLogger.info(TAG, "Flutter 未初始化，启动应用");
            // 启动应用并标记为从小组件启动
        ...
        FlutterMethodChannel.isRunning(isRunning -> {
            if (isRunning) {
                NativeLogger.info(TAG, "当前已连接，执行断开操作");
                // 立即更新UI为未连接状态
                ...
                NativeLogger.info(TAG, "当前未连接，执行连接操作");
                // 立即更新UI为已连接状态
                updateAllWidgetsWithState(context, true);
                // 执行连接操作
                FlutterMethodChannel.startVnt(null, success -> {
                    NativeLogger.info(TAG, "连接结果: " + success);
                    // 根据实际连接结果更新UI状态
                    if (success) {
                        NativeLogger.info(TAG, "连接成功");
                        updateAllWidgetsWithState(context, true);
                    } else {
                        NativeLogger.warning(TAG, "连接失败");
                        updateAllWidgetsWithState(context, false);
                    }
                    // 更新通知栏状态
                    MyVpnService.updateForegroundNotification();
        ...
        if (FlutterMethodChannel.initialized()) {
            ...
            if (!defaultKey.isEmpty()) {
                try {
                    ...
                    NativeLogger.error(TAG, "读取配置失败", e);
                }
            }

...
public class TileConfigActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(configNames, selectedIndex, (dialog, which) -> {
                ...
                NativeLogger.info(TAG, "用户选择配置: " + selectedConfig.name + " (key: " + selectedConfig.key + ")");

                // 立即连接该配置（不保存为默认配置）
                if (!FlutterMethodChannel.initialized()) {
                    NativeLogger.info(TAG, "Flutter 未初始化，启动应用并传递配置key");
                    ...
                    NativeLogger.info(TAG, "Flutter 已初始化，调用 startVnt 连接配置: " + selectedConfig.key);
                    // Flutter已初始化，直接调用连接，传递配置key
                    // Flutter端会自动处理断开当前连接并连接新配置的逻辑
                    FlutterMethodChannel.startVnt(selectedConfig.key, isRunning -> {
                        NativeLogger.info(TAG, "连接配置 [" + selectedConfig.name + "] - isRunning: " + isRunning);
                        // 更新磁贴和小组件状态                
    ...
    private List<ConfigItem> loadConfigs() {
        ...
        try {
            ...
            // 调试：打印所有键
            NativeLogger.debug(TAG, "所有 SharedPreferences 键: " + prefs.getAll().keySet());

            // 读取供原生代码使用的 JSON 格式配置列表
            String jsonArrayStr = prefs.getString(DATA_KEY_NATIVE, null);
            NativeLogger.debug(TAG, "读取到的配置数据 (DATA_KEY_NATIVE=" + DATA_KEY_NATIVE + "): " + jsonArrayStr);

            if (jsonArrayStr != null && !jsonArrayStr.isEmpty()) {
                // 这是一个 JSON 数组字符串，包含多个配置的 JSON 字符串
                JSONArray jsonArray = new JSONArray(jsonArrayStr);
                NativeLogger.debug(TAG, "解析后的 JSON 数组长度: " + jsonArray.length());

                for (int i = 0; i < jsonArray.length(); i++) {
                    String configJsonStr = jsonArray.getString(i);
                    NativeLogger.debug(TAG, "配置 " + i + ": " + configJsonStr);
                    ...
                    if (!key.isEmpty()) {
                        ...
                        NativeLogger.debug(TAG, "添加配置: key=" + key + ", name=" + name);
                    }
                }
            } else {
                NativeLogger.warning(TAG, "未找到配置数据 (键名: " + DATA_KEY_NATIVE + ")");
            }
        } catch (Exception e) {
            NativeLogger.error(TAG, "读取配置列表失败", e);
        }
        NativeLogger.debug(TAG, "最终读取到 " + configs.size() + " 个配置");
        return configs;
    }
    ...
    private String loadDefaultKey() {
        ...
        NativeLogger.debug(TAG, "读取默认配置 key (DEFAULT_KEY=" + DEFAULT_KEY + "): " + defaultKey);

        // 如果没有读��到，尝试不带前缀的键名
        if (defaultKey.isEmpty()) {
            defaultKey = prefs.getString("default-key", "");
            NativeLogger.debug(TAG, "尝试读取 default-key: " + defaultKey);
        }
    ...
    private void saveDefaultKey(String key) {
        ...
        NativeLogger.info(TAG, "已设置默认配置: " + key);
    }
